...
operstion on relations: deriving relations
...
parrallel processing: relations in and out: pipeline:piping the outout of one op into the next; partition: N op-clones, each processes 1/N input
...
So, the **`reason`** is, keras in TitanX is too recent (version `1.2.2`) for Fuxin's code, which is only compatible to older version(like `0.3.2` in pelican), and the output dimension of AveragePooling2D function is different between the two versions.
